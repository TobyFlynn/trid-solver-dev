...
void tridSInit(tridS_handle &handle, trid_mpi_handle &mpi_handle, int ndim, int *size) {
  ...
  handle.ndim = ndim;
  handle.pdims = (int *) calloc(handle.ndim, sizeof(int));
  mpi_handle.periodic = (int *) calloc(handle.ndim, sizeof(int));; //false
  mpi_handle.coords = (int *) calloc(handle.ndim, sizeof(int));
  MPI_Dims_create(mpi_handle.procs, handle.ndim, handle.pdims);
  
  // Create cartecian mpi comm
  MPI_Cart_create(MPI_COMM_WORLD, handle.ndim, handle.pdims, mpi_handle.periodic, 0,  &(mpi_handle.comm));
  
  // Get rand and coord of current mpi proc
  MPI_Comm_rank(comm, &(mpi_handle.my_cart_rank));
  MPI_Cart_coords(comm, mpi_handle.my_cart_rank, handle.ndim, mpi_handle.coords);
  ...
  for(int i = 0; i < handle.ndim; i++) {
    handle.sys_len_l[i] = handle.pdims[i] * 2;
    handle.n_sys_g[i] = 1;
    handle.n_sys_l[i] = 1;
  ...
  int max = 0;
  for(int i = 0; i < handle.ndim; i++) {
    if(handle.n_sys_l[i] > max) {
      max = handle.n_sys_l[i];
    }
  }
  
  handle.halo_sndbuf = (float *) _mm_malloc(max * max * 3 * sizeof(float), SIMD_WIDTH);
  handle.halo_rcvbuf = (float *) _mm_malloc(max * max * 3 * sizeof(float), SIMD_WIDTH);
  ...
  handle.aa_r = (float *) _mm_malloc(sizeof(float) * max * max, SIMD_WIDTH);
  handle.cc_r = (float *) _mm_malloc(sizeof(float) * max * max, SIMD_WIDTH);
  handle.dd_r = (float *) _mm_malloc(sizeof(float) * max * max, SIMD_WIDTH);
}
...
void tridDInit(tridD_handle &handle, trid_mpi_handle &mpi_handle, int ndim, int *size) {
  ...
  handle.halo_sndbuf = (double *) _mm_malloc(max * max * 3 * sizeof(double), SIMD_WIDTH);
  handle.halo_rcvbuf = (double *) _mm_malloc(max * max * 3 * sizeof(double), SIMD_WIDTH);
  ...
  handle.aa_r = (double *) _mm_malloc(sizeof(double) * max * max, SIMD_WIDTH);
  handle.cc_r = (double *) _mm_malloc(sizeof(double) * max * max, SIMD_WIDTH);
  handle.dd_r = (double *) _mm_malloc(sizeof(double) * max * max, SIMD_WIDTH);
}
...
void tridBatchS(tridS_handle &handle, trid_mpi_handle &mpi_handle, int solveDim) {
  if(solveDim == 0) {
    ...
    for(int id = 0; id < handle.n_sys_g[0]; id++) {
      ...
      handle.halo_sndbuf[halo_base + 1] = handle.aa[data_base + app.nx-1];
      handle.halo_sndbuf[halo_base + 2] = handle.cc[data_base];
      handle.halo_sndbuf[halo_base + 3] = handle.cc[data_base + app.nx-1];
      handle.halo_sndbuf[halo_base + 4] = handle.dd[data_base];
      handle.halo_sndbuf[halo_base + 5] = handle.dd[data_base + app.nx-1];
    }
    ...
    if(mpi_handle.coords[0] == 0) {
      #pragma omp parallel for
      for(int id = 0; id < app.n_sys_l[0]; id++) {
        int base = id * app.sys_len_l[0];
    ...
    for(int id = 0; id < handle.n_sys_g[0]; id++) {
      ...
      handle.dd[data_base + app.nx-1] = handle.halo_sndbuf[halo_base + 1];
    }
    ...
    if(mpi_handle.coords[1] == 0) {
      #pragma omp parallel for
      for(int id = 0; id < app.n_sys_l[1]; id++) {
